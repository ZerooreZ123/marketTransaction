...
class Axis extends Component {
    ...
    let isEnd = this.props.isEnd && this.props.isEnd() || true;
		if (showTicks && isEnd) {
			const tickProps = tickHelper(this.props, getScale(moreProps));
      tickProps.tickLineStroke = this.props.tickLineStroke;
      tickProps.tickLineStrokeOpacity = this.props.tickLineStrokeOpacity;
...
Axis.propTypes = {
	...
	tickStroke: PropTypes.oneOfType([PropTypes.string, PropTypes.func]),
	tickLineStroke: PropTypes.oneOfType([PropTypes.string, PropTypes.func]),
	tickStrokeOpacity: PropTypes.oneOfType([PropTypes.number, PropTypes.func]),
	tickLineStrokeOpacity: PropTypes.oneOfType([PropTypes.number, PropTypes.func]),
...
function drawTicks(ctx, result) {
	...
	const { tickStroke, tickLineStroke, tickStrokeOpacity, tickLabelFill, tickLineStrokeOpacity } = result;
	...
	if (showTickLabel) {
      ...
      let value = result.format(tick.value);
      if(value !== ''){
        ctx.fillStyle = tickLineStroke.constructor === Function? tickLineStroke(index): tickLineStroke;
        ctx.strokeStyle = hexToRGBA(ctx.fillStyle, tickLineStrokeOpacity.constructor === Function? tickLineStrokeOpacity(index): tickLineStrokeOpacity);
        drawEachTick(ctx, tick, result);
        ctx.fillStyle = tickStroke.constructor === Function? tickStroke(index): tickStroke;
        ctx.strokeStyle = hexToRGBA(ctx.fillStyle, tickStrokeOpacity.constructor === Function? tickStrokeOpacity(index): tickStrokeOpacity);
        drawEachTickLabel(ctx, tick, result, value);
      }
		});
	}
}
...
function drawEachTickLabel(ctx, tick, result, value) {
	const { canvas_dy } = result;

	ctx.beginPath();
	ctx.fillText(value, tick.labelX, tick.labelY + canvas_dy);
}
