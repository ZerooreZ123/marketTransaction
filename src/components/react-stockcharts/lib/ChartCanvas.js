...
class ChartCanvas extends Component {
    ...
    this.mouseMoveNext = this.mouseMoveNext.bind(this);
    this.mouseMovePrev = this.mouseMovePrev.bind(this);
    ...
    this.props[`${type}Handle`] && this.props[`${type}Handle`](props);
  ...
  mouseMoveNext(e){
    console.log('mouseMoveNext');
    const { currentItem, mouseXY } = this.mutableState;
    const { plotData, xScale, xAccessor, yScale } = this.state;
    const newItemIndex = Math.max(xAccessor(currentItem) - 1, 0);
    const domain = xScale.domain();
    domain[0] > newItemIndex && this.xAxisZoom(domain.map(index => index - 1))
    this.handleMouseMove([xScale(newItemIndex), mouseXY[1]], 'mousemove', e);
  }
  mouseMovePrev(e){
    console.log('mouseMovePrev');
    const { currentItem, mouseXY } = this.mutableState;
    const { plotData, xScale, xAccessor, yScale } = this.state;
    const newItemIndex = Math.min(xAccessor(currentItem) + 1, this.fullData.length - 1);
    const domain = xScale.domain();
    domain[1] < newItemIndex && this.xAxisZoom(domain.map(index => index + 1))
    this.handleMouseMove([xScale(newItemIndex), mouseXY[1]], 'mousemove', e);
  }
