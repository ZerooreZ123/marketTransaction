...
export default function fitWidth(WrappedComponent, withRef = true, minWidth = 100, minHeight = 100) {
	class ResponsiveComponent extends Component {
        ...
        height: 0
				...
				const el = this.node;
				const { width, paddingLeft, paddingRight, height } = window.getComputedStyle(el.parentNode);
        ...
        const h = parseFloat(height);
          ...
          height: Math.round(Math.max(h, minHeight))
				});
			});
		}
		getWrappedInstance() {
    ...
    componentWillReceiveProps(nextProps) {
      if(nextProps.showFive !== this.props.showFive || this.props.sign !== nextProps.sign){
        this.handleWindowResize();
      }
    }
		render() {
			...
			if (this.state.width) {
				return <WrappedComponent width={this.state.width} height={this.state.height} ratio={this.state.ratio} {...this.props} {...ref} />;
			} else {
				return <div {...ref}>
